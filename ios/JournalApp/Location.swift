...
*/
   ...
   @objc func fetchPhotos2(_ cameraRollOnly: Bool) {
    testIdentifiers = []
...
//     let cameraRollCollection = PHAssetCollection.fetchAssetCollections(with: .smartAlbum, subtype: .smartAlbumUserLibrary, options: PHFetchOptions()).firstObject
     ...
     albums.enumerateObjects { collection, _, _ in
       ...
       fetchedAssets.append(contentsOf: assetResult.objects(at: IndexSet(0..<assetResult.count)))
...
//    } 
    ...
    self.sendEvent(withName: "photoCount", body: fetchedAssets.count)
     for (index, asset) in fetchedAssets.enumerated() {
       self.sendEvent(withName: "photoChange", body: index)

       var newPhoto = [String: String]()
      newPhoto["name"] = PHAssetResource.assetResources(for: asset).first?.originalFilename
...
//            print(newPhoto["description"] ?? "")
  ...
  @objc func fetchPhotos3(_ cameraRollOnly: Bool) {
 testIdentifiers = []
 let fetchOptions = PHFetchOptions()
  let requestOptions = PHImageRequestOptions()
  requestOptions.isSynchronous = true
